...
function check_update()
		...
		downloadToFile(url, path, function(type, pos, total_size)
			if type == "finished" then
				...
				if updateInfo then
					...
					print("[Goss Helper] Òåêóùàÿ óñòàíîâëåííàÿ âåðñèÿ:", thisScript().version)
					print("[Goss Helper] Òåêóùàÿ âåðñèÿ â îáëàêå:", uVer)
					if thisScript().version ~= uVer then
						print('[Goss Helper] Äîñòóïíî îáíîâëåíèå!')
						sampAddChatMessage('[Goss Helper] {ffffff}Äîñòóïíî îáíîâëåíèå!', message_color)
						...
						print('[Goss Helper] Îáíîâëåíèå íå íóæíî!')
						sampAddChatMessage('[Goss Helper] {ffffff}Îáíîâëåíèå íå íóæíî, ó âàñ àêòóàëüíàÿ âåðñèÿ!', message_color)
					end
				end
			end
		end)
	...
	function readJsonFile(filePath)
		if not doesFileExist(filePath) then
			print("[Goss Helper] Îøèáêà: Ôàéë " .. filePath .. " íå ñóùåñòâóåò")
			return nil
		end
		...
		if not jsonData then
			print("[Goss Helper] Îøèáêà: Íåâåðíûé ôîðìàò JSON â ôàéëå " .. filePath)
			return nil
		end
		return jsonData
	end
...
function downloadFileFromUrlToPath(url, path)
	...
	if isMonetLoader() then
		downloadToFile(url, path, function(type, pos, total_size)
			...
			elseif type == "error" then
				sampAddChatMessage('[Goss Helper] {ffffff}Îøèáêà çàãðóçêè: ' .. pos,  message_color)
			end
		end)
	else
		downloadUrlToFile(url, path, function(id, status)
			...
			if imgui.BeginTabItem(fa.STAR .. u8' Ðîçûñê è øòðàôû') then 
				if imgui.BeginChild('##smartuk', imgui.ImVec2(292 * settings.general.custom_dpi, 340 * settings.general.custom_dpi), true) then
					...
					if imgui.Button(fa.DOWNLOAD .. u8' Çàãðóçèòü ##smartuk') then
						...
						downloadFileFromUrlToPath('https://github.com/MTGMODS/Rodina-helper/raw/refs/heads/main/SmartUK/' .. getARZServerNumber() .. '/SmartUK.json', path_uk)
					...
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' Goss Helper - Îïîâåùåíèå##donwloadsmartuk', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
						...
						if imgui.Button(fa.CIRCLE_PLAY .. u8' Îòêðûòü îáëàêî', imgui.ImVec2(300 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							openLink('https://github.com/MTGMODS/Rodina-helper/raw/refs/heads/main/SmartUK/')
							MainWindow[0] = false
						end
						imgui.EndPopup()
					end
				...
				if imgui.BeginChild('##smartpdd', imgui.ImVec2(292 * settings.general.custom_dpi, 340 * settings.general.custom_dpi), true) then
					...
					if imgui.Button(fa.DOWNLOAD .. u8' Çàãðóçèòü ##smartpdd') then
						...
						downloadFileFromUrlToPath('https://github.com/MTGMODS/Rodina-helper/raw/refs/heads/main/SmartPDD/' .. getARZServerNumber() .. '/SmartPDD.json', path_pdd)
					...
					if imgui.BeginPopupModal(fa.CIRCLE_INFO .. u8' Goss Helper - Îïîâåùåíèå##donwloadsmartpdd', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
						...
						imgui.SameLine()
						if imgui.Button(fa.CIRCLE_PLAY .. u8' Îòêðûòü îáëàêî', imgui.ImVec2(300 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							openLink('https://github.com/MTGMODS/Rodina-helper/raw/refs/heads/main/SmartPDD/')
							MainWindow[0] = false
						end
						imgui.EndPopup()
					end
