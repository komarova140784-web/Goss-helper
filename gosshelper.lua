...
-- скрипт был переделан MTG

script_name("Goss Helper")
script_author("Fil автор MTG")
script_version("2.2")
...
print('[Goss Helper] Скрипт запущен. Версия: ' .. thisScript().version)
-------------------------------------------- JSON SETTINGS ---------------------------------------------
...
local commands = {
	commands = {
		...
		{ cmd = 'zd' , description = 'Приветствие игрока' , text = 'Здраствуйте {get_ru_nick({arg_id})}&Я {my_ru_nick} - {accent} {fraction_tag}&Чем я могу Вам помочь?', arg = '{arg_id}' , enable = true , waiting = '2.200', bind = "{}" },
		...
		{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на свои часы с гравировкой  is fil и посмотрел{sex} время&/time&/do На часах показано время {get_time}.' , arg = '' , enable = true, waiting = '2.200' , bind = "{}"  },
        ...
        { cmd = 'pas' , description = 'Запросить документы' ,  text = 'Здраствуйте, я {accent} {fraction_tag}&/do Cлева на груди спец-жетон {fraction_tag}.&/me указывает пальцем на свой спец-жетон на груди&Прошу предъявить документ, удостоверяющий вашу личность.&/n {get_nick({arg_id})}, введите /showpass {my_id} или /showbadge {my_id}' , arg = '{arg_id}' , enable = true , waiting = '2.200' , bind = "{}"  },
        ...
        { cmd = 'pursuit' , description = 'Погоня за игроком' ,  text = '/me достаёт свой КПК и заходит в базу данных {fraction_tag}&/me открывает дело гражданина N и нажимает на кнопку отслеживание местоположения&/pursuit&/me выходит с базы данных {fraction_tag} и выключив КПК убирает его на поясной держатель' , arg = '' , enable = true, waiting = '2.200' , bind = "{}"  },
		...
		{ cmd = 'sbor' , description = 'Строй' ,  text = '/r Строй на парковке готовность 5 минут если вас нет выг/увал&/r Строй на парковке готовность 4 минут если вас нет выг/увал&/r Строй на парковке готовность 3 минут если вас нет выг/увал&/r Строй на парковке готовность 2 минут если вас нет выг/увал&/r Строй на парковке готовность 1 минут если вас нет выг/увал' , arg = '' , enable = true, waiting = '60.000' , bind = "{}"  },
		...
		{ cmd = 'se' , description = 'Кричалка' ,  text = '/s Всем поднять руки вверх, работает {fraction_tag}!', arg = '', enable = true, waiting = '2.200', bind = "{}" },
		...
		{ cmd = 'arrest' , description = 'Посадить в КПЗ',  text = '/me открыл дверь КПЗ ключом и взяв за шкирку подозреваемого, зашвырнул его во внутрь&/do Человек в КПЗ.&/arrest {arg_id}', arg = '{arg_id}', enable = true, waiting = '2.200', bind = "{}", in_fastmenu = true },
...
local checkbox_awanted = imgui.new.bool(settings.general.auto_find_wanteds or true)
...
local input_fraction_tag = imgui.new.char[256](u8(settings.player_info.accent))
...
local tagReplacements = {
	...
	accent = function() return settings.player_info.accent end,
...
{accent} - Ваша фракционная должность
...
function initialize_commands()
	...
	sampRegisterChatCommand("lekc", function(arg)
		if not isActiveCommand then
			if isParamSampID(arg) then
				...
				sampAddChatMessage('[Goss Helper] {ffffff}Используйте ' .. message_color_hex .. '/lekc и любую цыфру', message_color)
				play_error_sound()
			end	
		else
			sampAddChatMessage('[Goss Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
	...
	sampRegisterChatCommand("awanted", function() 
		for i = 1, 10, 1 do
			sampAddChatMessage('[Goss Helper] {ffffff}Данная функция доступна только в платной версии хелпера! Покупать у , Fil', message_color)
		end
	end)
	...
	sampRegisterChatCommand("wanteds", function(arg)
		...
		elseif not isActiveCommand then
			lua_thread.create(function()
				sampAddChatMessage('[Goss Helper] {ffffff}Смотрите внимательно как скрипт не выводит в чат', message_color)
...
local english_characters = {
    [168] = 'Yo', [184] = 'yo', [192] = 'A', [193] = 'B', [194] = 'V', [195] = 'G', [196] = 'D', 
    [197] = 'E', [198] = 'Zh', [199] = 'Z', [200] = 'I', [201] = 'Y', [202] = 'K', [203] = 'L', 
    [204] = 'M', [205] = 'N', [206] = 'O', [207] = 'P', [208] = 'R', [209] = 'S', [210] = 'T', 
    [211] = 'U', [212] = 'F', [213] = 'Kh', [214] = 'Ts', [215] = 'Ch', [216] = 'Sh', [217] = 'Shch', 
    [218] = '', [219] = 'Y', [220] = '', [221] = 'E', [222] = 'Yu', [223] = 'Ya', [224] = 'a', 
    [225] = 'b', [226] = 'v', [227] = 'g', [228] = 'd', [229] = 'e', [230] = 'zh', [231] = 'z', 
    [232] = 'i', [233] = 'y', [234] = 'k', [235] = 'l', [236] = 'm', [237] = 'n', [238] = 'o', 
    [239] = 'p', [240] = 'r', [241] = 's', [242] = 't', [243] = 'u', [244] = 'f', [245] = 'kh', 
    [246] = 'ts', [247] = 'ch', [248] = 'sh', [249] = 'shch', [250] = '', [251] = 'y', [252] = '', 
    [253] = 'e', [254] = 'yu', [255] = 'ya'
}
...
function calculateZoneRu(x, y, z)
    local streets = {
        ...
        {"", 1817.390, 863.232, -89.084, 2027.390, 1083.230, 110.916},
...
function check_update()
    ...
    sampAddChatMessage('[Goss Helper] {ffffff}Начинаю проверку на наличие обновлений...', message_color)
	    ...
	    print('[Goss Helper] Разработчик Andrey_Fil ')
    sampAddChatMessage('[Goss Helper] {ffffff}Разработчик Andrey_Fil ', message_color)
	    print('[Goss Helper] Разработчик Andrey_Fil ')
    sampAddChatMessage('[Goss Helper] {ffffff}Разработчик Andrey_Fil ', message_color)
	    print('[Goss Helper] Разработчик Andrey_Fil ')
    sampAddChatMessage('[Goss Helper] {ffffff}Разработчик Andrey_Fil ', message_color)
...
function sampev.onServerMessage(color,text)
	...
	if (text:find('Bogdan_Martelli%[%d+%]') and getARZServerNumber():find('20')) or text:find('%[20%]Bogdan_Martelli') then
		...
		if text:find('%[20%]Bogdan_Martelli%[%d+%]') then
			...
			text = string.gsub(text, '%[20%]Bogdan_Martelli%[%d+%]', message_color_hex .. '[20], Fil[' .. id .. ']' .. lastColor)
		
		elseif text:find('%[20%]Bogdan_Martelli') then
			-- Случай 1: [20]Bogdan_Martelli
			text = string.gsub(text, '%[20%]Bogdan_Martelli', message_color_hex .. '[20], Fil' .. lastColor)
		
		elseif text:find('Bogdan_Martelli%[%d+%]') then
			-- Случай 3: Bogdan_Martelli[123]
			local id = text:match('Bogdan_Martelli%[(%d+)%]') or ''
			text = string.gsub(text, 'Bogdan_Martelli%[%d+%]', message_color_hex .. ', Fil[' .. id .. ']' .. lastColor)
		elseif text:find('Bogdan_Martelli') then
			-- Случай 3: Bogdan_Martelli
			text = string.gsub(text, 'Bogdan_Martelli', message_color_hex .. ', Fil' .. lastColor)
		end
		return {color,text}
	end
end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if title:find('Основная статистика') and check_stats then -- получение статистики
		...
		if text:find("{FFFFFF}Должность: {B83434}(.+)%((%d+)%)") then
			settings.player_info.accent, settings.player_info.fraction_rank_number = text:match("{FFFFFF}Должность: {B83434}(.+)%((%d+)%)(.+)Уровень розыска")
			sampAddChatMessage('[Goss Helper] {ffffff}Ваша должность обнаружена, это: '..settings.player_info.accent.." ("..settings.player_info.fraction_rank_number..")", message_color)
			if tonumber(settings.player_info.fraction_rank_number) >= 9 then
				settings.general.auto_uval = true
				initialize_commands()
			end
		else
			settings.player_info.accent = "Неизвестно"
		...
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 169 * settings.general.custom_dpi), true) then
					...
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton(u8'должность##accent') then
...
if imgui.BeginPopupModal(fa.BUILDING_SHIELD .. u8' Тэг организации##fraction_tag', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
    ...
    imgui.End()
					...
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton(u8'не работает##fraction') then
					end
					...
					if imgui.BeginPopupModal(fa.BUILDING_SHIELD .. u8' Тэг организации##fraction', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
						change_dpi()
						imgui.PushItemWidth(405 * settings.general.custom_dpi)
						imgui.InputText(u8'##input_fraction', input_fraction, 256)
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. u8' Отмена', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
						imgui.SameLine()
						if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							settings.player_info.fraction = u8:decode(ffi.string(input_fraction))
							save_settings()
							imgui.CloseCurrentPopup()
						end
						imgui.End()
					...
					imgui.NextColumn()
					imgui.CenterColumnText(u8(settings.player_info.accent) .. " (" .. settings.player_info.fraction_rank_number .. ")")
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton(u8"Не работает##rank") then
					end
					imgui.Columns(1)
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 53 * settings.general.custom_dpi), true) then
					imgui.CenterText(fa.ROBOT .. u8' Ассистент(не ворк)')
					imgui.Separator()
					imgui.Columns(2)
				...
				if imgui.BeginChild('##4', imgui.ImVec2(589 * settings.general.custom_dpi, 28 * settings.general.custom_dpi), true) then
					...
					if imgui.BeginPopupModal(fa.SACK_DOLLAR .. u8'', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						if imgui.Button(u8(''), imgui.ImVec2(imgui.GetMiddleButtonX(2), 25 * settings.general.custom_dpi)) then
							openLink('')
						end
						imgui.Separator()
				...
				if imgui.BeginTabBar('Tabs2') then
					if imgui.BeginTabItem(fa.BARS..u8' Стандартные команды') then 
						if imgui.BeginChild('##99', imgui.ImVec2(589 * settings.general.custom_dpi, 333 * settings.general.custom_dpi), true) then
							...
							imgui.NextColumn()
							imgui.CenterColumnText(u8"Меню лекций и разговоров")
							imgui.Columns(1)
							imgui.Separator()
							imgui.Columns(2)
					...
					if imgui.BeginTabItem(fa.BARS..u8' RP команды (общие)') then 
						...
						if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую команду##new_cmd',imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
							if #commands.commands >= 500 then
								for i = 1, 10, 1 do
									sampAddChatMessage('[Goss Helper] {ffffff}Лимит FREE версии на 500 команд, безлим в платной версии хелпера! Покупать у , Fil', message_color)
								end
							else
					...
					if imgui.BeginTabItem(fa.BARS..u8' RP команды (для 9/10)') then 
						if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
							...
							if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую команду##new_cmd_9-10', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
								...
								if #commands.commands_manage >= 500 then
									for i = 1, 10, 1 do
										sampAddChatMessage('[Goss Helper] {ffffff}Лимит FREE версии на 500 лидер.команд, безлим в платной версии хелпера! Покупать у , Fil', message_color)
									end
								else
									local new_cmd = {cmd = '', description = '', text = '', arg = '', enable = true, waiting = '2.200', bind = "{}" }
			...
			if imgui.BeginTabItem(fa.STAR .. u8' Розыск и штрафы') then 
				...
				imgui.CenterText(u8'Облачные розыск/штрафы устарели? Сообщите в бота @Adofibot разработчик исправит.')
			imgui.EndTabItem()
			end
			if imgui.BeginTabItem(fa.FILE_PEN..u8' Заметки') then 
				...
				if imgui.Button(fa.CIRCLE_PLUS .. u8' Создать новую заметку', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					if #notes.note >= 500 then
						for i = 1, 10, 1 do
							sampAddChatMessage('[Goss Helper] {ffffff}Лимит FREE версии на 500 заметок, безлим в платной версии хелпера! Покупать у , Fil', message_color)
						end
					else
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				...
				imgui.Separator()
				imgui.Text(fa.CIRCLE_USER..u8" Разработчик данного хелпера: Andrey_Fil")
				imgui.Separator()
				imgui.Text(fa.CIRCLE_INFO..u8" Установленная версия хелпера: " .. u8(thisScript().version))
				imgui.SameLine()
				if imgui.SmallButton(u8'Не работает') then
...
imgui.OnFrame(
    ...
    function(player)
		...
		if imgui.Button(fa.CLOCK .. u8' Задержка',imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			imgui.OpenPopup(fa.CLOCK .. u8' Задержка (в Милисекунды) ')
		end
		if imgui.BeginPopupModal(fa.CLOCK .. u8' Задержка (в Милисекунды) ', _, imgui.WindowFlags.NoResize ) then
...
imgui.OnFrame(
    ...
    function(player)
	  ...
	  --imgui.Text(fa.CITY .. u8(' Город: ') .. u8(tagReplacements.get_city()))
		imgui.Text(fa.MAP_LOCATION_DOT .. u8(' Айди: ') .. u8(tagReplacements.my_id()))
		imgui.Text(fa.MAP_LOCATION_DOT .. u8(' Ник: ') .. u8(tagReplacements.my_nick()))
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			...
			if imgui.Button(fa.PLAY .. u8" Начать собеседование", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
					sampSendChat("Здравствуйте, я " .. settings.player_info.name_surname .. " - " .. settings.player_info.accent .. ' ' .. settings.player_info.fraction_tag)
					wait(2000)
			...
			if imgui.BeginChild('sobes3', imgui.ImVec2(150 * settings.general.custom_dpi, -1), true) then
				...
				if imgui.Selectable(u8"Нету паспорта") then
					lua_thread.create(function ()
						...
						wait(2000)
						sampSendChat("Получите паспорт в Мэрии.")
					end)
				end
				...
				if imgui.Selectable(u8"Нету военного билета") then
					lua_thread.create(function ()
                        ...
                        wait(2000)
						sampSendChat("/n Получить его можно отслужив в армии ")
					end)
				end
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(fa.PERSON_CIRCLE_CHECK..u8' "Лекции" ' .. sampGetPlayerNickname(player_id), lekcMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
			change_dpi()
			if imgui.BeginChild('lekc1', imgui.ImVec2(240 * settings.general.custom_dpi, 182 * settings.general.custom_dpi), true) then
			imgui.CenterColumnText(fa.BOOKMARK .. u8" Лекции")
			imgui.Separator()
			if imgui.Button(fa.CHECK .. u8"Собурдинация", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
					sampSendChat("Уважаемые сотрудники")
					wait(1000)
					...
					wait(1000)
					sampSendChat("За работу!")

				end)
			end
			if imgui.Button(fa.CHECK .. u8"Обязаности Сотрудника", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
					sampSendChat("Хорошо, предоставьте мне все ваши документы для проверки.")
					wait(1000)
				end)
			end
			if imgui.Button(fa.CHECK .. u8"Оперативные транспортные средства.", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
				sampSendChat("Уважаемые сотрудники")
				wait(1000)
				sampSendChat("Сегодня мы поговорим на тему 'Оперативные транспортные средства.'")
				wait(1000)
				sampSendChat("В автопарке ФСБ могут быть и другие виды транспорта, например БТРы,")
				wait(1000)
				sampSendChat("...Range Rover Sport, volkswagen passat и транспортно-воздушные средства")
				wait(1000)
				sampSendChat("...для передвижения.")
				wait(1000)
				sampSendChat("Информацию о средствах передвижения ФСБ часто держат в секрете,")
				wait(1000)
				sampSendChat("...так как деятельность службы подразумевает высокий уровень скрытности.")
				wait(1000)
				sampSendChat("Благодарю за прослушивание моей лекции, на этом все.")
				wait(1000)
				sampSendChat("За работу!")			
			...
			if imgui.Button(fa.CHECK .. u8"Маскировка", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
				sampSendChat("Уважаемые сотрудники")
				wait(1000)
				sampSendChat("Сейчас я проведу лекцию на тему 'Маскировка'.")
				wait(1000)
				sampSendChat("Маскировка это форма сотрудников другой организации или группировки для внедрения сотрудника ФСБ")
				wait(1000)
				sampSendChat("Маскировкой могут пользоваться сотрудники 5-го порядкового звания!")
				wait(1000)
				sampSendChat("Маскировка применяется для слежки за другими организациями или группировками!")
				wait(1000)
				sampSendChat("Маскировку нельзя использовать без разрешения Старшего Состава!")
				wait(1000)
				sampSendChat("Нельзя надевать форму СС или лидера организации!")
				wait(1000)
				sampSendChat("За нарушение правил маскировки вы получите наказание вплоть до увольнения!")
				wait(1000)
				sampSendChat("Благодарю за прослушивание моей лекции, на этом все.")
				wait(1000)
				sampSendChat("За работу!")
			...
			if imgui.Button(fa.CHECK .. u8"Правила строя", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
				sampSendChat("Сейчас я проведу лекцию на тему 'Правила строя'.")
				wait(1000)
				...
				wait(1000)
				sampSendChat("В ФСБ как и в армии вы должны соблюдать строгую субординацию.")
				wait(1000)
				sampSendChat("Это подразумевает: не хамить старшим, не пререкаться с ними, не перебивать.")
				wait(1000)
				sampSendChat("В ФСБ нет слов 'Можно, нельзя, да, не знаю, спасибо, сэр.'")
				wait(1000)
				...
				wait(1000)
				sampSendChat("Кто ослушается - будет уволен и занесён в черный список ФСБ.")
				wait(1000)
				sampSendChat("Использование оружия разрешено только при ЧС и тренировках.")
				wait(1000)
				sampSendChat("Для всех работа с 10:00 до 20:30 - Будни, с 10:00 до 20:00 - выходные.")
				wait(1000)
				sampSendChat("Спать вам разрешено только в раздевалке.")
				wait(1000)
				sampSendChat("Благодарю за прослушивание моей лекции, на этом все.")
				wait(1000)
				sampSendChat("За работу!")
			...
			imgui.SameLine()
			if imgui.BeginChild('sobes2', imgui.ImVec2(240 * settings.general.custom_dpi, 182 * settings.general.custom_dpi), true) then
				imgui.CenterColumnText(fa.BOOKMARK..u8"Разговор")
				imgui.Separator()
				if imgui.Button(fa.GLOBE .. u8"Наркотические вещества", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
					sampSendChat("Уважаемые сотрудники")
					wait(1000)
					sampSendChat("Если у вас находятся наркотические вещества то вы можите получить увольнение/выговор")
					wait(1000)
					sampSendChat("Если вам выпадют с рулеток немедленно выкинуть в мусорку иначе получите")
					wait(1000)
					sampSendChat("А если будите употреблять то будите уволены")
					wait(1000)
					sampSendChat("И если будет проверка то старший состав получит тоже")
					wait(1000)
					sampSendChat("Так что будьте внимательны и не нарушайте ")
					wait(1000)
					end)
				end
				if imgui.Button(fa.CIRCLE_QUESTION .. u8"Материалы", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
					sampSendChat("Уважаемые сотрудники")
					wait(1000)
					sampSendChat("Если у вас находятся материалы то вы можите получить увольнение/выговор")
					wait(1000)
					sampSendChat("Если вам выпадют с рулеток немедленно выкинуть в мусорку иначе получите")
					wait(1000)
					sampSendChat("А если будите употреблять то будите уволены")
				...
				if imgui.Button(fa.CIRCLE_QUESTION .. u8"Нарушение", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
					lua_thread.create(function()
					sampSendChat("Уважаемые сотрудники")
					wait(1000)
					sampSendChat("Если вы будите нарушать вы можите получить увольнение/выговор зависит от вас")
					wait(1000)
					sampSendChat("А так же если увидят старший состав то вам не поздаровится")
					wait(1000)
					sampSendChat("Если вы не знаете Устав ФП или еще что то, то вас не освобождает от отвественности")
					wait(1000)
					sampSendChat("Так что следите за собой и не нарушайте")
					wait(1000)
					end)
				end
			imgui.EndChild()
