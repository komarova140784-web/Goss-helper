...
Вот модифицированный код, который выполняет проверку обновления только один раз при запуске игры:
...
-- Теперь вызываем функцию проверки обновления только при запуске
addEventHandler('onClientResourcesStart', root,
    function()
        check_update()  -- Вызываем проверку при старте ресурсов
    end
)

-- Или если используется MoonLoader:
-- addEventHandler('onClientResourceStart', root, check_update)
