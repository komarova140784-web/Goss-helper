...
function check_update()
		...
		downloadToFile(url, path, function(type, pos, total_size)
			if type == "finished" then
				...
				if updateInfo then
					...
					print("[Goss Helper] Текущая установленная версия:", thisScript().version)
					print("[Goss Helper] Текущая версия в облаке:", uVer)
					if thisScript().version ~= uVer then
						print('[Goss Helper] Доступно обновление!')
						sampAddChatMessage('[Goss Helper] {ffffff}Доступно обновление!', message_color)
						...
						print('[Goss Helper] Обновление не нужно!')
						sampAddChatMessage('[Goss Helper] {ffffff}Обновление не нужно, у вас актуальная версия!', message_color)
					end
				end
			end
		end)
	...
	function readJsonFile(filePath)
		if not doesFileExist(filePath) then
			print("[Goss Helper] Ошибка: Файл " .. filePath .. " не существует")
			return nil
		end
		...
		if not jsonData then
			print("[Goss Helper] Ошибка: Неверный формат JSON в файле " .. filePath)
			return nil
		end
		return jsonData
	end
...
function downloadFileFromUrlToPath(url, path)
	...
	if isMonetLoader() then
		downloadToFile(url, path, function(type, pos, total_size)
			...
			elseif type == "error" then
				sampAddChatMessage('[Goss Helper] {ffffff}Ошибка загрузки: ' .. pos,  message_color)
			end
		end)
	else
		downloadUrlToFile(url, path, function(id, status)
